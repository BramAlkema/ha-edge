...
# Connects to Cloud Run edge server via chisel tunnel
#
# Features:
...
GENERATED_PASS_FILE="/config/.ha_edge_password"
LOG_FILE="/tmp/ha_edge.log"
...
mark_restart_needed() {
    touch "/config/.ha_edge_needs_restart"
}
...
setup_tunnel_sensors() {
    ...
    local sensor_file="/config/packages/ha_edge_sensors.yaml"

    cat > "$sensor_file" << 'EOF'
# Auto-generated by Home Assistant Edge add-on
# Provides sensors for edge status monitoring
...
template:
  - binary_sensor:
      - name: "HA Edge Connected"
        unique_id: ha_edge_connected
        device_class: connectivity
        state: "{{ state_attr('sensor.ha_edge_status', 'tunnel_connected') | default(false) }}"
        icon: mdi:tunnel

      - name: "HA Edge Report State"
        unique_id: ha_edge_report_state
        state: "{{ state_attr('sensor.ha_edge_status', 'report_state_enabled') | default(false) }}"
...
# Setup Alexa Smart Home package configuration
# Uses HA's built-in alexa.smart_home integration
setup_alexa() {
    local server_url
    ...
    bashio::log.info "Alexa: Setting up packages configuration..."

    # Create packages directory if it doesn't exist
    mkdir -p /config/packages

    # Write the alexa package
    local package_file="/config/packages/ha_edge_alexa.yaml"

    cat > "$package_file" << 'EOF'
# Auto-generated by Home Assistant Edge add-on
# Uses Home Assistant's built-in Alexa Smart Home integration
  ...
  smart_home:
    ...
    filter:
      include_domains:
        ...
        - vacuum
      exclude_entities: []
    # Entity configuration can be customized here
    # entity_config:
    #   light.kitchen:
    #     name: Kitchen Light
...
EOF
    ...
    bashio::log.info "Alexa: Package written to $package_file"
}
...
setup_google_assistant() {
    ...
    if [ -f "$sa_file" ]; then
        ...
        cp "$sa_file" /config/ha_edge_service_account.json
        chmod 600 /config/ha_edge_service_account.json
    ...
    local package_file="/config/packages/ha_edge_google_assistant.yaml"
...
EOF
    ...
    if [ "$has_service_account" = true ]; then
        cat >> "$package_file" << EOF
  service_account: !include ../ha_edge_service_account.json
  report_state: true
EOF
    ...
    if [ -f "/config/.ha_edge_needs_restart" ]; then
        rm -f "/config/.ha_edge_needs_restart"
...
main() {
    bashio::log.info "========================================"
    bashio::log.info "Home Assistant Edge starting"
    bashio::log.info "========================================"
